...
export class ActiveSpellParserService {
  ...
  private enrichAction(action: Action, previousAction: Action): Action {
    ...
    if (
      ...
      action.entities.get(action.entityId).getTag(GameTag.CARDTYPE) ===
        CardType.SPELL
    ) {
      activeSpell = action.entityId;
    } else if (
      action instanceof PowerTargetAction &&
      action.entities.get(action.originId).getTag(GameTag.CARDTYPE) ===
        CardType.SPELL
    ) {
      ...
      action.entities.get(action.origin).getTag(GameTag.CARDTYPE) ===
        CardType.SPELL
    ) {
      ...
      action instanceof AttachingEnchantmentAction &&
      previousAction &&
      previousAction.activeSpell
    ) {
      activeSpell = previousAction.activeSpell;
    } else if (
      action instanceof CardDrawAction &&
      previousAction &&
      previousAction.activeSpell
    ) {
      activeSpell = previousAction.activeSpell;
      ...
      action instanceof HealingAction &&
      previousAction &&
      previousAction.activeSpell
    ) {
      activeSpell = previousAction.activeSpell;
