...
export class ActionParserService {
  ...
  public *parseActions(
    ...
    for (const item of history) {
      ...
      actionParsers.forEach(parser => {
        if (parser.applies(item)) {
          ...
          if (actions && actions.length > 0) {
            actionsForTurn = this.fillMissingEntities(
              actionsForTurn,
              previousStateEntities
            );
      ...
      const [updatedTurn, newCurrentTurn] = this.updateCurrentTurn(
        item,
        game,
        actionsForTurn,
        currentTurn
      ...
      if (updatedTurn) {
        ...
        actionsForTurn = this.addDamageToEntities(
          actionsForTurn,
          previousStateEntities
        );
        ...
        const turnNumber =
          turnWithNewActions.turn === "mulligan"
            ? 0
            : parseInt(turnWithNewActions.turn);
    ...
    try {
      const turnWithNewActions = game.turns
        .get(currentTurn)
        .update({ actions: actionsForTurn });
      turns = turns.set(
        turnWithNewActions.turn === "mulligan"
          ? 0
          : parseInt(turnWithNewActions.turn),
  ...
  private addDamageToEntities(
    ...
    for (let i = 0; i < actionsForTurn.length; i++) {
      const newEntities = actionsForTurn[i].entities
        ...
        : previousStateEntities;
      const entitiesAfterDamageUpdate: Map<
        number,
        Entity
      > = this.isDamageAction(actionsForTurn[i])
        ? newEntities
            .map(entity =>
              this.updateDamageForEntity(actionsForTurn[i], entity)
            )
            .toMap()
        : newEntities;
      newActionsForTurn.push(
        actionsForTurn[i].update(entitiesAfterDamageUpdate)
      );
    }
    return newActionsForTurn;
  }
  ...
  private areEqual(
    actions1: readonly Action[],
    actions2: readonly Action[]
  ): boolean {
