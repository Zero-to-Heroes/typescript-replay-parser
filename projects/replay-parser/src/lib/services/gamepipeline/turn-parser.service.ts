...
export class TurnParserService {
  ...
  public createTurns(game: Game, history: readonly HistoryItem[]): Game {
	...
	for (const item of history) {
		...
		const mulliganTurn: MulliganTurn = this.parseMulliganTurn(
			item as TagChangeHistoryItem,
			turns
		);
		turns = turns.set(0, mulliganTurn);
		turnNumber++;
		} else if (this.isStartOfTurn(item, game)) {
		const turn: ActionTurn = this.parseTurn(
			turnNumber,
			item as TagChangeHistoryItem,
			turns
		);
  ...
  private parseTurn(
	...
	let turn: ActionTurn = turns.get(currentTurnNumber, Object.assign(
		new ActionTurn(),
		{
  ...
  private parseMulliganTurn(
	item: TagChangeHistoryItem,
	turns: Map<number, Turn>
  ): MulliganTurn {
	const itemIndex = (item as TagChangeHistoryItem).tag.index;
	let mulliganTurn: MulliganTurn = turns.get(0, Object.assign(
		new MulliganTurn(),
		{
		turn: 'mulligan',
		timestamp: item.timestamp,
