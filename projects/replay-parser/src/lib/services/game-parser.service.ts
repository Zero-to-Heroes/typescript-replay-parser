...
export class GameParserService {
  ...
  public async parse(
    ...
    const iterator: IterableIterator<
      [Game, number, string]
    > = this.createGamePipeline(replayAsString, start);
  ...
  private buildObservableFunction(
    observer,
    iterator: IterableIterator<[Game, number, string]>
  ) {
  ...
  private *createGamePipeline(
    ...
    const history: readonly HistoryItem[] = new XmlParserService(
      this.logger
    ).parseXml(replayAsString);
    ...
    const initialEntities = this.gamePopulationService.populateInitialEntities(
      history
    );
    ...
    const entities: Map<
      number,
      Entity
    > = this.gameStateParser.populateEntitiesUntilMulliganState(
      history,
      initialEntities
    );
    ...
    const gameWithPlayers: Game = this.gameInitializer.initializeGameWithPlayers(
      history,
      entities
    );
    ...
    const gameWithTurns: Game = this.turnParser.createTurns(
      gameWithPlayers,
      history
    );
    ...
    const gameWithFullStory: Game = this.narrator.createGameStory(
      gameWithNarrator
    );
