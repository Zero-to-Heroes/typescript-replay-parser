...
export class GamePopulationService {
  ...
  public populateInitialEntities(
    history: readonly HistoryItem[]
  ): Map<number, Entity> {
    // Map of entityId - entity definition
    const entities: Map<number, Entity> = Map();
    const entitiesAfterInit: Map<number, Entity> = this.initializeEntities(
      history,
      entities
    );
    const entitiesAfterMissingInfo: Map<
      number,
      Entity
    > = this.completeMissingInformation(history, entitiesAfterInit);
    const entitiesAfterBasicData: Map<number, Entity> = this.addBasicData(
      entitiesAfterMissingInfo
    );
  ...
  private initializeFullEntity(
    ...
    const entity: Entity = entities
      .get(
        historyItem.entityDefintion.id,
        Entity.create({ id: historyItem.entityDefintion.id } as Entity)
      )
  ...
  private addEntityInformation(
    item: ShowEntityHistoryItem,
    entities: Map<number, Entity>
  ): Map<number, Entity> {
    let result = entities;
    ...
    const newTags: Map<string, number> = Map<string, number>()
      .set(
        GameTag[GameTag.CREATOR],
        item.entityDefintion.tags.get(GameTag[GameTag.CREATOR])
      )
      .set(
    ...
    const finalEntity: Entity = result
      ...
      .map((value: Entity) => {
        ...
        if (card) {
          ...
          if (card.type === "Enchantment" && !value.getTag(GameTag.CARDTYPE)) {
            newTags = value.tags.set(
              GameTag[GameTag.CARDTYPE],
