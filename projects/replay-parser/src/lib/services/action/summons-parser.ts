...
export class SummonsParser implements Parser {
  ...
  public parse(
	...
	return entities
		...
		.filter(
		entity => entity.tags.get(GameTag[GameTag.CARDTYPE]) === CardType.MINION
		)
		.map(entity => {
  ...
  private shouldMergeActions(
	...
	if (
		!(previousAction instanceof SummonAction) ||
		!(currentAction instanceof SummonAction)
	) {
		return false;
	}
	if (
		(previousAction as SummonAction).origin !==
		(currentAction as SummonAction).origin
	) {
		return false;
	}
	return true;
  ...
  private mergeActions(
	previousAction: SummonAction,
	currentAction: SummonAction
  ): SummonAction {
	return SummonAction.create(
		{
		...
		entityIds: uniq([
			...uniq(previousAction.entityIds || []),
			...uniq(currentAction.entityIds || [])
		])
		},
		this.allCards
	);
