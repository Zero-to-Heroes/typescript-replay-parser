...
export class CardDrawParser implements Parser {
      ...
      if (
        ...
        if (!controller) {
          this.logger.error(
            'empty controller',
            item,
            entitiesBeforeAction.get(item.entityDefintion.id)
          );
        }
        return [
          CardDrawAction.create(
  ...
  private shouldMergeActions(previous: Action, current: Action): boolean {
    if (
      !(previous instanceof CardDrawAction && current instanceof CardDrawAction)
    ) {
      return false;
    }
    if (previous.controller === undefined || current.controller === undefined) {
      this.logger.error(
        '[card-draw-parser] Empty controller for draw action',
        previous,
        current
      );
    }
  ...
  private mergeActions(
    previousAction: CardDrawAction,
    currentAction: CardDrawAction
  ): CardDrawAction {
