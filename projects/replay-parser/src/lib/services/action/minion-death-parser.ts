...
export class MinionDeathParser implements Parser {
  ...
  public applies(item: HistoryItem): boolean {
    return (
      item instanceof ActionHistoryItem || item instanceof TagChangeHistoryItem
    );
  }
  ...
  public parse(
    ...
    if (item instanceof TagChangeHistoryItem) {
      ...
      const parentAction = history.find(
        historyItem => historyItem.index === parentActionId
      );
  ...
  public reduce(actions: readonly Action[]): readonly Action[] {
    return ActionHelper.combineActions<MinionDeathAction>(
      actions,
      (previous, current) =>
        previous instanceof MinionDeathAction &&
        current instanceof MinionDeathAction,
      (previous, current) => this.mergeActions(previous, current)
    );
  }

  private mergeActions(
    previousAction: MinionDeathAction,
    currentAction: MinionDeathAction
  ): MinionDeathAction {
