...
export class DamageParser implements Parser {
  ...
  public applies(item: HistoryItem): boolean {
	return (
		item instanceof TagChangeHistoryItem && item.tag.tag === GameTag.DAMAGE
	);
  }
  ...
  public parse(
	...
	const previousDamage =
		!previousDamageTag || previousDamageTag === -1 ? 0 : previousDamageTag;
  ...
  private mergeActions(previousAction: Action, currentAction: Action): Action {
	return this.mergeDamageIntoAction(
		previousAction,
		currentAction as DamageAction
	);
  }

  private mergeDamageIntoAction(
	previousAction: Action,
	currentAction: DamageAction
  ): Action {
	const result = ActionHelper.mergeIntoFirstAction(
		previousAction,
		currentAction,
		{
