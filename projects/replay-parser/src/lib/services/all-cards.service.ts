...
const CARDS_CDN_URL =
  "https://static.zerotoheroes.com/hearthstone/jsoncards/cards.json";
...
export class AllCardsService {
  ...
  public async initializeCardsDb(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      if (this.allCards) {
        this.logger.debug(
          "[all-cards] already loaded all cards",
          this.allCards
        );
        resolve();
        return;
      ...
      this.http.get("./cards.json").subscribe(
        (result: any[]) => {
          this.logger.debug(
            "[all-cards] retrieved all cards locally",
            result.length
          );
        ...
        error => {
          this.logger.debug(
            "[all-cards] Could not retrieve cards locally, getting them from CDN",
            error
          );
          this.http.get(CARDS_CDN_URL).subscribe(
            (result: any[]) => {
              this.logger.debug(
                "[all-cards] retrieved all cards from CDN",
                result.length
              );
              this.allCards = result;
              resolve();
            ...
            error => {
              this.logger.debug(
                "[all-cards] Could not retrieve cards from CDN",
                error
              );
              reject();
            }
